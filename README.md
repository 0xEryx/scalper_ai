...
This directory contains the current scalper-oriented agent skill bundle built on top of OKX Agent Trade Kit.

The goal of this bundle is to add a trader workflow layer that can:
...
This bundle is currently a v2 alpha. It is suitable for local testing, demo trading, and playbook iteration. It is not yet a production-ready real-money trading system.
...
The bundle consists of four custom agent skills:
...
The current v2 alpha schema supports:
...
This means the current version supports:

- simple price-triggered management rules
- stop-triggered management rules
- lightweight lifecycle-based rules
- a first stateful two-step sequence shape such as:
  - "if price reaches A, then wait for B, then add to the same-side position"

It still does not support broader multi-stage state machines such as:
...
Those patterns are exactly the kind of thing we believe later playbook versions should make much easier to express on top of OKX AI tooling.
...
- add same-side position by `notional_usdt`, `size_pct`, or `margin_pct * leverage`
- emit notify-only actions for trader-facing follow-up reporting

### 4. Position Watch

`scalper-position-watch` is the watcher layer.

The current version now uses a script-first watcher architecture:

- `run_watch_cycle.py`
  - runs one deterministic watch cycle
  - loads the playbook
  - resolves a live ticker or uses a provided snapshot
  - evaluates rules without needing the LLM in the loop
  - dispatches supported actions directly
- `run_watch_service.py`
  - wraps a single cycle as a background-friendly service entrypoint
  - emits a structured watcher event only when something actually triggers
  - writes service events to `.scalper-runtime/events/watch-events.jsonl`
- `notify_watch_event.py`
  - bridges a triggered watcher event back into the local agent messaging layer
  - lets the LLM wake up only when a meaningful event needs a user-facing update

This means the watcher path is now closer to:

- scheduler / cron / service
- script watcher logic
- action dispatch
- optional local message bridge only when an event fires

instead of forcing the LLM to participate on every watch loop.

It is still an alpha workflow component, but it is materially stronger than the earlier "watcher as a thin helper script" version.

## What This Bundle Depends On

You need all of the following on the local machine:

- a compatible local agent runtime
- Python 3
- OKX Trade CLI
- a valid local OKX config in `~/.okx/config.toml`
- a local runtime workspace at `~/.openclaw/workspace`
...
## Install The Skills Into Your Local Workspace
...
This is the runtime workspace used by the local agent environment in the current setup.
...
### Step 3. Run A Script-First Watch Cycle

If you already have a saved playbook and want to test the watcher without an LLM in the loop, run:
...
This writes cycle output to:

```text
~/.openclaw/workspace/.scalper-runtime/logs/watch-cycles.jsonl
```

### Step 4. Run The Watch Service And Event Bridge

To test the service wrapper and notification bridge:
...
This writes structured triggered events to:

```text
~/.openclaw/workspace/.scalper-runtime/events/watch-events.jsonl
```

If you want a one-shot path for local flow testing, run:
...
### Step 2b. Compile A v2 Sequence Playbook

This bundle now includes a minimal v2-style sequence example:
...
That example represents a two-step rule shape:

- first reach one price
- then wait for a rebound
- then add to the same-side position
...
## Example Of A Supported v2 Alpha Strategy
...
Another supported v2 alpha shape is:

- first reach 2122
- then rebound to 2144
- then add 3% same-side short exposure at 20x

This is supported because the schema now includes:
...
## Example Of A Strategy That Is Not Yet Fully Supported
...
Those are planned for later schema versions, not the current v2 alpha.
...
The custom skills are already usable through the current local agent runtime, but the routing is still alpha-quality.
...
## Current v2 Alpha Focus

The active development direction is:

- broader playbook schema coverage
- stronger stateful sequence support
- lifecycle-driven watcher events
- richer executor action coverage
- clearer trader-facing reporting while keeping deterministic internal JSON contracts

## Safety Notes

- Use `demo` first.
- Do not treat this v2 alpha as production-ready.
