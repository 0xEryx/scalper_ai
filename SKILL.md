...
This top-level router skill should usually not produce a long standalone report unless the user is asking about the whole workflow.

When it does respond directly, it should summarize:

- which sub-skill the request should route to
- why that route was chosen
- whether the request is supported in the current MVP

Preferred format:

```text
🧭 Scalper Agent Router
━━━━━━━━━━━━━━━━━━━━
路由结果：scalper-playbook
原因：请求包含条件语句与后续管理逻辑
状态：Supported / Partially Supported / Unsupported
━━━━━━━━━━━━━━━━━━━━
➡️ 下一步：进入 playbook 编译
```

If the request exceeds the current MVP, say so directly instead of pretending the full workflow can proceed.
