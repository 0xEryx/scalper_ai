...
2. If required fields are missing, stop and tell the user the request is not executable in the current version.
3. For entry orders, run `scripts/place_entry_order.py`.
4. For follow-up actions, run `scripts/dispatch_action.py`.
5. Support only the current action set and refuse unsupported strategy writes.
6. Log any successful write into `.scalper-runtime/logs/orders.jsonl`.
...
- If the action cannot be mapped to the supported action set, say that this version does not support it yet.
- Never guess price, side, size, or profile.

## Supported v2 Alpha Actions
